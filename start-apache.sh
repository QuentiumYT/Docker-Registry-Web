...
echo "{\"host\": \"$ENV_REGISTRY_PROXY_FQDN\", \"port\": $ENV_REGISTRY_PROXY_PORT}" > /var/www/html/app/registry-host.json

# information about browse mode.
[[ x$ENV_MODE_BROWSE_ONLY =~ ^x(true|false)$ ]] || ENV_MODE_BROWSE_ONLY=false
[[ x$ENV_MODE_REPO_DELETE =~ ^x(true|false)$ ]] || ENV_MODE_REPO_DELETE=false
[[ -z "$ENV_DEFAULT_REPOSITORIES_PER_PAGE" ]] && ENV_DEFAULT_REPOSITORIES_PER_PAGE=10
[[ -z "$ENV_DEFAULT_TAGS_PER_PAGE" ]] && ENV_DEFAULT_TAGS_PER_PAGE=5
echo "{\"browseOnly\":$ENV_MODE_BROWSE_ONLY, \"repoDelete\":$ENV_MODE_REPO_DELETE, \"defaultRepositoriesPerPage\":$ENV_DEFAULT_REPOSITORIES_PER_PAGE, \"defaultTagsPerPage\":$ENV_DEFAULT_TAGS_PER_PAGE}" > /var/www/html/app/app-mode.json
...
if [ -n "$ENV_USE_SSL" ]; then
    ...
    [[ ! -e /certs/registry.crt ]] && die "/etc/apache2/server.crt is missing"
    [[ ! -e /certs/registry.key ]] && die "/etc/apache2/server.key is missing"
    needModSsl=1
fi
